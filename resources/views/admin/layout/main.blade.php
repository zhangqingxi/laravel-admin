...
<head>
    ...
    <link rel="stylesheet" href="{{asset('admin/common/style.css')}}" media="all"/>
    ...
    <script>
        //一般直接写在一个js文件中
        let ROOT_URL = "{{url('admin').'/'}}";

        let ROOT_COMMON_URL = "{{url('admin/common').'/'}}";
        //定义layuiadmin入口
        let ASSET_LAY = "{{asset('admin/layuiadmin').'/'}}";

        layui.config({
            base: ASSET_LAY,//静态资源所在路径*-
        }).extend({ //设定模块别名
            index: 'lib/index', //主入口模块
            common: '{/}{{asset("admin/common/common")}}',
        }).use(['index'], function () {
            layer.config({
